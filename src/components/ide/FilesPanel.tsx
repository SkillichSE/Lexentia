...
export function FilesPanel({ onChange }: { onChange: (files: PendingFile[]) => void }) {
  ...
  const setNext = (next: PendingFile[]) => {
    setFiles(next)
    onChange(next)
  }
  ...
  const removeFile = (id: string) => setNext(files.filter((f) => f.id !== id))
  const clearAll = () => setNext([])

  return (
    <div className="lex-filesPanel">
      <div className="lex-filesHeader">
        <div className="lex-subtle">{fileCountLabel}</div>
        {files.length ? (
          <button className="lex-btn lex-btn--small" onClick={clearAll}>
            Clear
          </button>
        ) : null}
      </div>
        ...
        {files.length === 0 ? (
          ...
          files.map((f) => (
            <div className="lex-fileRow" key={f.id}>
              ...
              <button className="lex-btn lex-btn--icon" onClick={() => removeFile(f.id)} title="Remove file">
                ×
              </button>
            </div>
          ))

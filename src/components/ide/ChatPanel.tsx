...
import type { LexAction, PlanResult, PlanStep } from '../../services/modelService'
import { CodeAnimationPanel } from './CodeAnimationPanel'
...
export function ChatPanel({
  ...
  plansByMessageId?: Record<string, PlanResult | undefined>
  onApprovePlan?: (messageId: string, plan: PlanResult) => void
}) {
                    ...
                    {actionsByMessageId[m.id]!
                      ...
                      .map((a, idx) => (
                        <FileWritePreview
                          ...
                          onProgress={(p) => setFilePreviewProgress((prev: Record<string, number>) => ({ ...prev, [m.id]: p }))}
                        />
                      ))}
                  </div>
                ) : null}
...
function PlanPreview({
  ...
  return (
    <div className="lex-planPreview">
      <div className="lex-planHeader">
        <div className="lex-planTitle">📋 {plan.title}</div>
        <div className="lex-planRight">
          {plan.totalEstimatedTime && (
            <div className="lex-subtle">⏱️ {plan.totalEstimatedTime}</div>
          )}
          <button className="lex-btn lex-btn--primary" onClick={onApprove}>
            Approve Plan
          </button>
          <button className="lex-btn" type="button" onClick={() => setExpanded(!expanded)}>
            {expanded ? 'Hide' : 'Show'}
          </button>
        </div>
      </div>
      <div className="lex-planDescription">{plan.description}</div>
      {expanded && (
        <div className="lex-planSteps">
          {plan.steps.map((step, idx) => (
            <div key={step.id} className="lex-planStep">
              <div className="lex-stepHeader">
                <div className="lex-stepNumber">{idx + 1}</div>
                <div className="lex-stepTitle">{step.title}</div>
                {step.estimatedTime && (
                  <div className="lex-stepTime">⏱️ {step.estimatedTime}</div>
                )}
              </div>
              <div className="lex-stepDescription">{step.description}</div>
              {step.dependencies && step.dependencies.length > 0 && (
                <div className="lex-stepDependencies">
                  <span className="lex-subtle">Dependencies: </span>
...
function FileWritePreview({
  ...
  }, [content, messageId, open, progress, onProgress])
    ...
    <div className="lex-filePreview" aria-label={`File preview ${relPath}`}>
      <div className="lex-filePreviewHeader">
        ...
        <div className="lex-filePreviewRight">
          ...
          <button className="lex-btn" type="button" onClick={() => setOpen((p: boolean) => !p)}>
            {open ? 'Hide' : 'Show'}
          </button>
        </div>
      </div>

...
function runGit(args) {
  return new Promise((resolve) => {
    ...
    const proc = (0, import_node_child_process.spawn)(cmd, args, { cwd: workspaceRoot, shell: isWin, env: process.env });
    let output = "";
    let error = "";
...
async function registerIpcHandlers() {
  ...
  import_electron.ipcMain.handle("extension:getInstalled", async () => {
    return { ok: true, installed: [] };
  });
  import_electron.ipcMain.handle("extension:getCommands", async () => {
    return { ok: true, commands: [] };
  });
  import_electron.ipcMain.handle("extension:executeCommand", async () => {
    return { ok: false, error: "Extensions support disabled" };
  });
  import_electron.ipcMain.handle("extension:install", async () => {
    return { ok: false, error: "Extensions support disabled" };
  });
  import_electron.ipcMain.handle("term:create", async (evt, opts) => {
    ...
    try {
      ...
      p.onData((data) => {
        win.webContents.send("term:data", { id, data: encodeTermChunk(data) });
      });
      p.onExit(() => {
        terminals.delete(id);

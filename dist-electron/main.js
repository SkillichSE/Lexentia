...
function registerIpcHandlers() {
  ...
  import_electron.ipcMain.handle("workspace:newFile", async (evt) => {
    if (!workspaceRoot) return { ok: false, error: "No workspace folder open" };
    const win = import_electron.BrowserWindow.fromWebContents(evt.sender);
    const res = await import_electron.dialog.showSaveDialog(win ?? void 0, {
      title: "New File",
      buttonLabel: "Create",
      defaultPath: workspaceRoot
    });
    if (res.canceled || !res.filePath) return { ok: false };
    const abs = import_node_path.default.normalize(res.filePath);
    if (!isWithinWorkspace(abs)) {
      return { ok: false, error: "Choose a location inside the workspace folder" };
    }
    const relRaw = import_node_path.default.relative(import_node_path.default.normalize(workspaceRoot), abs);
    const relPath = relRaw.split(import_node_path.default.sep).join("/");
    try {
      ...
      if (!import_node_fs.default.existsSync(abs)) {
        await import_promises.default.writeFile(abs, "", "utf-8");
      }
    } catch (e) {
  ...
  import_electron.ipcMain.handle("workspace:pickFile", async (evt) => {
    if (!workspaceRoot) return { ok: false, error: "No workspace folder open" };
    const win = import_electron.BrowserWindow.fromWebContents(evt.sender);
    const res = await import_electron.dialog.showOpenDialog(win ?? void 0, {
      title: "Open File",
      defaultPath: workspaceRoot,
      properties: ["openFile"]
    });
    if (res.canceled || res.filePaths.length === 0) return { ok: false };
    const abs = import_node_path.default.normalize(res.filePaths[0]);
    try {
    ...
    if (!isWithinWorkspace(abs)) {
      return { ok: false, error: "Choose a file inside the workspace folder" };
    }
    const relRaw = import_node_path.default.relative(import_node_path.default.normalize(workspaceRoot), abs);
    const relPath = relRaw.split(import_node_path.default.sep).join("/");
    return { ok: true, relPath };

...
function registerIpcHandlers() {
  ...
  ipcMain.handle('workspace:newFile', async (evt) => {
    ...
    const abs = path.normalize(res.filePath)
    if (!isWithinWorkspace(abs)) {
      return { ok: false, error: 'Choose a location inside the workspace folder' }
    }
    const relRaw = path.relative(path.normalize(workspaceRoot), abs)
  ...
  ipcMain.handle('workspace:pickFile', async (evt) => {
    if (!workspaceRoot) return { ok: false, error: 'No workspace folder open' }
    const win = BrowserWindow.fromWebContents(evt.sender)
    ...
    if (res.canceled || res.filePaths.length === 0) return { ok: false }
    const abs = path.normalize(res.filePaths[0])
    try {
